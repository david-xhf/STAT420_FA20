...
predict(co2_slr, impreza_wagon, interval = "prediction", level = 0.95)
```

- This model doesn't seem to fit the data well. Based on the plot, we can see the line is consistently underestimating `CO2` for both `Both` and `Truck` type vehicles.

- Using this model, an estimate for the average change in `CO2` for a one foot-pound per second increase in `horse` for a vehicle of type `truck` is given by `r slope_truck`.

- The 95% prediction interval for the Subaru Impreza Wagon is: (`r predict(co2_slr, impreza_wagon, interval = "prediction", level = 0.95)[2:3]`).
...
predict(co2_add, impreza_wagon, interval = "prediction", level = 0.95)
```
...
- The 95% prediction interval for the Subaru Impreza Wagon is: (`r predict(co2_add, impreza_wagon, interval = "prediction", level = 0.95)[2:3]`).
...
predict(co2_int, impreza_wagon, interval = "prediction", level = 0.95)
```
...
- The 95% prediction interval for the Subaru Impreza Wagon is: (`r predict(co2_int, impreza_wagon, interval = "prediction", level = 0.95)[2:3]`).
...
```{r, solution = TRUE}
anova(co2_slr, co2_add)
```

Here we see an extremely small p-value, so we reject the null hypothesis. Thus, we prefer the larger, additive model.
...
```{r, solution = TRUE}
anova(co2_slr, co2_int)
```

Here again, we see an extremely small p-value, so we reject the null hypothesis. Thus, we prefer the larger, interaction model.
...
anova(hosp_add_care_int, hosp_add_care_race_int)
```

Based on the p-value of $`r anova(hosp_add_care_int, hosp_add_care_race_int)[2,]$"Pr"`$ we reject the null. We prefer the larger model with the added interaction terms.

**Note:** The original intention was to compare the the previous model, which the code for is above. As stated, the correct code is below.
...
coef(hosp_add_care_race_int)[c(3, 7, 9)]
sum(coef(hosp_add_care_race_int)[c(3, 9)])
```

Based on this model, an estimate for the change in average `Days` for a one-unit increase in `Pressure` for a `"white"` patient that required a high level of care is given by `r sum(coef(hosp_add_care_race_int)[c(3, 9)])`.
...
Then choose the correction coefficient.
...
\[
(\beta_1 + \beta_4 \times 20  + \beta_5 \times 10 + \beta_7 \times 20 \times 10) = (\beta_1 + \beta_4 \times 20  + \beta_5 \times 10 + \beta_7 \times 200)
\]
