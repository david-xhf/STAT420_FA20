...
Recall how `R` deals with vectors of different lengths. What is actually happening with  `sum_of_power(x = a, p = c(1, 2))`?

\[
1 ^ 1 + 2 ^ 2 + 3 ^ 1 + 4 ^ 2 + 5 ^ 1 + 6 ^ 2 + 7 ^ 1 + 8 ^ 2 + 9 ^ 1 + 10 ^ 2
\]

`R` first repeats the vector `c(1, 2)` to match the length of `a`, then each element of `a` is raised to the corresponding power from `c(1, 2, 1, 2, 1, 2, 1, 2, 1, 2)`.
...
Notice the value `385` keeps appearing!
...
```{r, solution = TRUE}
(iq_t_test = t.test(iq ~ town, data = intelligence, alternative = "greater"))
```

The p-value of the test is **`r iq_t_test$p.value`** so we **fail to reject** at a significance level of $\alpha = 0.10$. Thus, people from Eagleton are **not** smarter than people from Pawnee.

Note that using `alternative = "greater"` is correct since `R` picks the "groups" alphabetically. Then Eagleton is $X$ and Pawnee is $Y$ in the classic two-sample setup. So, since the sample mean for Eagleton is higher, and the p-value is less than 0.5, we can be certain we have entered the test correctly.

Also note that this code by default does not make an equal variance assumption. You could justify the use of equal variance with this data.
...
Only the p-value, which is doubled. The test statistic and our decision remains the same.
...
Here we see two possible functions. Notice that both deal with the issue of the possibility that $t$ could be positive or negative.
...
do_t_test = function(x, mu = 0) {
  ...
  t_abs = abs(t)
  pval  = 2 * pt(t_abs, df = n - 1, lower.tail = FALSE)
  c(t, pval)
}
```
...
do_t_test = function(x, mu = 0) {
  n    = length(x)
  t    = (mean(x) - mu) / (sd(x) / sqrt(n))
  pval = 2 * min(pt(t, df = n - 1, lower.tail = FALSE), pt(t, df = n - 1))
  c(t, pval)
}
```
...
We can use `t.test()` in `R` to verify our p-value.
...
x_bars = rep(0, num_samples)
for (i in 1:num_samples) {
  x_bars[i] = mean(rexp(n = sample_size, rate = lambda))
}
...
Based on this histogram, which is right skewed, we do **not** believe the CLT applies with a sample size of 5.
...
x_bars = replicate(num_samples, mean(rexp(n = sample_size, rate = lambda)))
...
Based on this histogram, which appears very normal, we believe the CLT applies with a sample size of 100.
...
```{r, solution = TRUE}
sim_xbars_exp = function(num_samples = 1000, sample_size = 10, lambda = 1) {
  
  replicate(num_samples, mean(rexp(n = sample_size, rate = lambda)))
  
}

x_bars = sim_xbars_exp(num_samples = 25000, sample_size = 50, lambda = 3)
...
```{r, solution = TRUE}
set.seed(birthday)
sims = replicate(5000, sd(rexp(n = 10, rate = 2)) / median(rpois(n = 5, lambda = 3)))
hist(sims, breaks = 25, 
     xlab = expression("Simulated values of " ~ frac(s[x], m[y])),
     main = "Histogram of Simulated Values",
     col = "darkorange",
     border = "dodgerblue")
```

This is an extremely right-skewed histogram.

Note that, with this particular seed for randomization, we obtained an interesting result, which prevents us from calculating the mean and standard deviation of the empirical distribution. During one of the simulations `median(rpois(n = 5, lambda = 3)` was `0`, meaning each of the simulated observations was `0`. Then, we are dividing by `0` in the statistic, so `R` stores a result of `Inf`.
