...
Set a seed of `114` and then split the `Boston` data into two datasets, one called `train_data` and one called `test_data`. The `train_data` dataframe should contain 400 randomly chosen observations. `test_data` will contain the remaining observations. Hint: consider the following code:
...
Report the diagonal of `C` as well as the 10th row of `ex_4_data`. For this exercise we will use the seed `114`.
